...
class SpeakerCriterion(BaseCriterion):
    ...
    def __init__(self, dimEncoder, nSpeakers):
        ...
        self.lstm = nn.LSTM(dimEncoder, dimEncoder, 4, batch_first=True)
        self.linearSpeakerClassifier1 = nn.Linear(
            dimEncoder, dimEncoder)
        self.linearSpeakerClassifier2 = nn.Linear(
    ...
    def forward(self, cFeature, otherEncoded, label):
        ...
        out, (hFeature, c_n) = self.lstm(cFeature)
        hFeature=hFeature[-1,:,:]
        predictions = self.linearSpeakerClassifier1(hFeature)
        predictions = self.linearSpeakerClassifier2(predictions)
